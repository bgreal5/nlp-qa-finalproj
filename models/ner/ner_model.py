...
class NERModel(nn.Module):
    ...
    [Architecture]
        ...
        2.5) NER Encoder we added to add additional embedding info
    ...
    def __init__(self, args):
        ...
        self.question_rnn = rnn_cell(
            args.embedding_dim + self.ner_encoder.ner_size,
            args.hidden_dim,
            bidirectional=args.bidirectional,
            batch_first=True,
        )
    ...
    def forward(self, batch):
        ...
        # 2.5) Use created NER embeddings (one hot) for both p and q from dataloader,
        ...
        # [batch_size, p_len, p_hid]
        passage_hidden = self.dropout(passage_hidden)
        ...
        question_vector = question_scores.unsqueeze(
            1).bmm(question_hidden).squeeze(1)
...
def _sort_batch_by_length(tensor, sequence_lengths):
    ...
    index_range = sequence_lengths.data.clone().copy_(
        torch.arange(0, len(sequence_lengths))).long()
